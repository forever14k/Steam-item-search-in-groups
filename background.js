...
function ShowUi() {
		...
		'<style>'+
			...
			'.friendBlock { transition: background 500ms;}'+
		...
		$('.maincontent').prepend('<div class="graytab_body_wrapper"><div class="graytab_body" id=backpackscontent></div></div>');
...
function SortResult() {
	...
	var ingame = [];
	var offline = [];
	$('.backpack .user .friendBlock').each(function() {
		status = "offline";
		if ($(this).hasClass('online')) { status = "online"; }
		if ($(this).hasClass('in-game')) { status = "ingame"; }
		switch(status) {
			case "online":
...
function ShowResult(account_id,item_uid,desc) {
	if (typeof(results[account_id]) == "undefined")
	{
		...
		$('.friendBlock[data-miniprofile='+account_id+']').clone().appendTo('.user[account_id='+account_id+']');
		results[account_id] = {};
	}
...
function GetAllInventoris() {
	...
	$('.friendBlock').css('background','none');
